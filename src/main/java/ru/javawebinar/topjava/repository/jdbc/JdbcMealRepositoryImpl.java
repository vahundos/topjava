...
public class JdbcMealRepositoryImpl implements MealRepository {
    ...
    public Meal save(Meal meal, int userId) {
        ...
        if (meal.isNew()) {
            ...
            int rowsAffected = namedParameterJdbcTemplate.update("UPDATE meals SET date_time=:date_time," +
    ...
    public List<Meal> getBetween(LocalDateTime startDate, LocalDateTime endDate, int userId) {
        Object[] params = {userId, startDate, endDate};
        int[] types = {Types.INTEGER, Types.TIMESTAMP, Types.TIMESTAMP};
        return jdbcTemplate.query("SELECT * FROM meals WHERE user_id = ? AND date_time BETWEEN ? AND ? ORDER BY " +
                "date_time DESC", params, types, ROW_MAPPER);
    }
}
